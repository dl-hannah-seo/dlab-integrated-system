...
export interface LabWeeklyMetric {
  ...
  payment_collection_rate: number; // 수강료 수납률(%) — 청구 대비 수납
}
...
const labBase = (
  ...
  attend: number, mkCount: number, mkDone: number, withdraw: number, payment: number,
): LabMetricBase => ({
  ...
  withdraw_rate: withdraw, payment_collection_rate: payment,
});

// 이번 주 명시값 — 시연용. 판교랩: 홍보 많지만 문의 전환 낮고 출결↓·퇴원율↑ 이상치.
const LAB_CURRENT: Record<string, LabMetricBase> = {
  //                  sns blog inq new re  att mk  mkD wd  pay
...
function genLabMetric(labIdx: number, weekIdx: number): LabMetricBase {
  ...
  return labBase(
    ...
    78 + h(10) * 2,    // payment collection 78..98
  );
}
