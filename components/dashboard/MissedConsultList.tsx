...
export function MissedConsultList({ limit = 5 }: { limit?: number }) {
  ...
  return (
    <Card title={`상담 미결 ${entries.length}명`}>
      {shown.length === 0 ? (
        ...
        <ul className="-my-1 space-y-1">
          {shown.map(l => (
            <li key={l.id} className="flex items-center justify-between gap-3 py-1.5">
              <span className="truncate text-sm text-[#37352F]">{l.name}</span>
              <Link href="/leads" className="shrink-0 text-xs font-medium text-[#FF6C37] hover:underline">
                자세히보기
              </Link>
            </li>
          ))}
