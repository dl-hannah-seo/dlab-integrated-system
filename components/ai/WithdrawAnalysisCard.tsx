    ...
    <Card>
      <div className="flex items-start justify-between mb-3">
        <div className="flex items-center gap-2">
          ...
          <h3 className="text-base font-semibold text-[#37352F]">퇴원 사유 분석</h3>
          <span className="text-xs px-2 py-0.5 bg-[#F7F7F5] border border-[#E9E9E7] rounded-full text-[#787774]">원생관리 연동</span>
        </div>
        {generated ? (
      ...
      {!generated ? (
        <div className="bg-[#F7F7F5] rounded-lg p-8 text-center border border-dashed border-[#E9E9E7]">
          <p className="text-sm text-[#787774]">버튼을 눌러 퇴원 사유 분석을 실행하세요</p>
        </div>
      ) : total === 0 ? (
        <p className="py-6 text-center text-sm text-[#787774]">퇴원 데이터가 없습니다.</p>
      ) : (
        <div className="space-y-4">
          <p className="text-xs text-[#787774]">누적 퇴원 <span className="text-[#37352F] font-medium">{total}명</span> · 사유별 분포</p>
            ...
            {counts.map(c => (
              <div key={c.reason}>
                <div className="flex items-center justify-between mb-0.5">
                  <span className="text-xs text-[#37352F]">{c.reason}</span>
                  <span className="text-xs text-[#787774] tabular-nums">{c.count}명 · {Math.round((c.count / total) * 100)}%</span>
                </div>
                <div className="h-2 rounded-full bg-[#F1F0EF]">
                  <div className="h-full rounded-full bg-[#EB5757]" style={{ width: `${(c.count / max) * 100}%` }} />
                </div>
              </div>
            ))}
          </div>
          ...
          <div>
            ...
            <div className="px-3 py-2.5 rounded-lg bg-[#FDECEA] border border-[#EB575726] text-xs leading-relaxed">
              ...
              <p className="mt-1 text-[#37352F]">→ {ACTION[top.reason] ?? '사유 패턴을 추가 분석하세요.'}</p>
            </div>
            <p className="text-[11px] text-[#BEBDBA] mt-2">원생관리에 기입된 퇴원 사유 기반 · 데이터 축적 후 Claude API 예측으로 확장 예정</p>
          </div>
        </div>
      )}
    </Card>
